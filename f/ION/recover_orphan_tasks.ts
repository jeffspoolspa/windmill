...
// Recover task-less ("orphan") visits: per distinct ion_task_id (EventID), read the customer from
// the service log (addLog -> CustomerID), pull task detail, create the task + schedules, link visits.
// Committing + idempotent (skips EventIDs whose task already exists). Batched, highest visit-count
// first; re-run advances. ADR-006 / docs/operations/ion-visit-task-backfill.md.
export async function main(limit = 250) {
  const r: any = await wmill.getResource("u/carter/supabase")
  const sql = postgres({ host: r.host, port: r.port, database: r.dbname, username: r.user, password: r.password, ssl: "require", max: 3, idle_timeout: 20, connect_timeout: 15 })
  ...
  const slByCust = new Map((await sql`
      select csa.customer_id, min(sl.id) as sl
      from public.customer_service_addresses csa
      join public.service_locations sl on sl.id = csa.service_location_id
      where csa.is_active and sl.geocode_status = 'ok' group by csa.customer_id`).map((x: any) => [Number(x.customer_id), Number(x.sl)]))
  ...
  const stats: any = { batch: targets.length, tasks_created: 0, schedules_created: 0, visits_linked: 0, customer_unmatched: 0, no_customerid_on_log: 0, errors: 0, examples: [] }
  ...
  for (const t of targets) {
    ...
    try {
      ...
      if (ex.length) {
        ...
        if (status === "active" && slId != null && openLocs.has(slId)) status = "closed"
        ...
        tid = (await sql`
          ...
          returning id`)[0].id
        stats.tasks_created++
        if (status === "active" && slId != null) openLocs.add(slId)
